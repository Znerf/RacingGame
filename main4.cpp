...
GLfloat xangle=0.0,yangle=-10.0,zangle=0.0,angle=0.0;   /* axis angles */
GLfloat modelx=0.0,modely=0.0,modelz=0.0;
...
GLvoid DrawGLScene()
{
    ...
    else
    {
        ...
        if(aflag==1)/* Initialize our window. */
            if(flag1==0)glClearColor(.5,.5,.5,1);
            else glClearColor(0.35,0.6,1,1);       //day ma color change garne sky ko
        else
            glClearColor(0.2,0.1,0.1,0);    //night color change garne sky ko
        glPushMatrix();
        glLoadIdentity();
        ...
        if(flag1)
         {
            ...
            glBegin(GL_POLYGON);
            glVertex3f(-200,0,0);
            glVertex3f(-200,0,0.8);
            glVertex3f(500,0,0.8);/////// road 1
            glVertex3f(500,0,0);
            glEnd();
            ...
            glVertex3f(-200,0,0);    ////////median
            glVertex3f(-200,0,0.2);
            glVertex3f(500,0,0.2);
            glVertex3f(500,0,0);
            glEnd();
            ...
            glColor3f(0.1,0.1,0.1);
            glVertex3f(-200,0,0);
            glVertex3f(-200,0,0.8);                     // road 2
            glVertex3f(500,0,0.8);
            glVertex3f(500,0,0);
            glEnd();
    ...
    switch ( key )    {
     case 27:
         if (start==0) exit(0);
                ...
                xt=0.0,yt=0.0,zt=0.0,xw=0.0;   // x,y,z translation factor
                tx=295,ty=62;
                xs=1.0,ys=1.0,zs=1.0;

                xangle=0.0,yangle=-5.0,zangle=0.0,angle=0.0;   /* axis angles */

                r=0,g=0,b=1;
                light=1;
                count=1,flg=1;
                start=0;
                flag1=0,aflag=1;            //to switch car driving mode
                flag2=0,wheelflag=1;   //to switch fog effect


                camxl=4,camyl=0,camzl=2;
                camx=1,camy=1,camz=1;
                camxL=0,camyL=1,camzL=0;

                modelx=0.0,modely=0.0,modelz=0.0;
...
void myMenu(int id)
{
   if(id==3)//fog
	...
	if(id==13)//night
	{
        ...
        flag2=2;
        glClearColor(0.1,0.1,0.1,0);
        GLfloat fogcolour[4]={0.06,0.06,0.06,1.0};

        glFogfv(GL_FOG_COLOR,fogcolour);              /* Define the fog colour */
        glFogf(GL_FOG_DENSITY,0.025);                   /* How dense */
        glFogi(GL_FOG_MODE,GL_EXP);                   /* exponential decay */
                             /* end */
        glHint(GL_FOG_HINT, GL_FASTEST);              /* compute per vertex */
        glEnable(GL_FOG);
...
int main(int argc, char **argv)
{
    ...
    glutTimerFunc(25, update, 0); //Add a timer      /* Function to do all our OpenGL drawing. */
