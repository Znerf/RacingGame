...
float camxl=10,camyl=3,camzl=2;
float camx=5,camy=2,camz=0;
        ...
        glPushMatrix();
           ...
           // glRotatef(zangle,0.0,0.0,1.0);
            ...
            gluLookAt(	camx, camy, camz,
                camx+camxl, camy+camyl,  camz+camzl,
                camxL, camyL,  camzL);
         //   std::cout<<camx<<' '<<camy<<" "<<camz<<camxl<<' '<<camyl<<" "<<camzl<<"\n";
...
static void SpecialKeyFunc( int Key, int x, int y )
{
	...
	case GLUT_KEY_DOWN:
		if(!wheelflag){
            xt -= 0.2;//*float(cos(yangle));
            zt += 0.2*float(sin(yangle));
            angle += 5;
		}
		if(wheelflag)
		{
